...
function DamageViewModel(props) {
    const { t, i18n } = useTranslation();
    const showEntityData = useSelector((state) => state.crudSlice.showEntityData)
    const theme = useMantineTheme();

    const closeModel = () => {
        props.setDamageViewModel(false)
    }

    return (
        <Modal opened={props.customerViewModel} onClose={closeModel} title={t('DamageDetailsData')} size="75%" overlayProps={{
...
export default DamageViewModel;
