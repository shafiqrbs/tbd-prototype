...
function MobileBankAccountViewModel(props) {
    const { t, i18n } = useTranslation();
    const showEntityData = useSelector((state) => state.crudSlice.showEntityData)
    const theme = useMantineTheme();

    const closeModel = () => {
        props.setMbAccountViewModel(false)
    }

    return (
        <Modal opened={props.MbAccountViewModel} onClose={closeModel} title={t('CustomerDetailsData')} size="75%" overlayProps={{
...
export default MobileBankAccountViewModel;
